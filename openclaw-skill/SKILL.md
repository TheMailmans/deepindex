---
name: embedcontext
description: Use EmbedContext CLI for semantic code search in any codebase. Use when: working on a coding task and need to find relevant files, understand architecture, locate TODOs, or trace a feature across layers. Requires embedcontext installed globally (npm install -g embedcontext) and Ollama running with nomic-embed-text. Do NOT use for simple one-file reads — just use the read tool directly.
---

# EmbedContext Skill

EmbedContext gives you semantic search over any codebase. It runs locally via Ollama — no cloud, no cost, fully offline.
...
Use `embedcontext search` for fast lookups when you don't need full MCP tool context:
...
# Filter to a domain
embedcontext search "error handling" --domain src

# More results
embedcontext search "database connection" --max-results 10
```
...
embedcontext index       # builds the vector index
embedcontext mcp-config  # outputs Claude Desktop config to paste
```
...
Working on a bug in EmbedContext itself:
...
4. <make the fix>
5. embedcontext index                  → re-index after changes
```
